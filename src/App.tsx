...
export default function App() {
  ...
  /** Phân hệ chỉ tồn tại ở sheet CONFIG. */
  const phanHeOptions = useMemo(() => {
    ...
    return [...counter.entries()].sort((a, b) => b[1] - a[1] || compare(a[0], b[0]));
  }, [configRecords]);

  /** Module dùng chung từ vựng ở cả hai sheet nên gộp lại một danh sách. */
  const moduleOptions = useMemo(() => {
    ...
    return [...counter.entries()].sort((a, b) => b[1] - a[1] || compare(a[0], b[0]));
  }, [configRecords, noteRecords, kindView, phanHeFilter]);
  ...
  const exportCsv = () => {
    if (kindView !== 'note' && filteredConfig.length) {
      downloadCsv(
        ...
        ['STT', 'Loại', 'Phân hệ', 'Mã Config', 'Module', 'Màn hình / Chức năng', 'Mô tả chức năng', 'Value'],
    ...
    if (kindView !== 'config' && filteredNotes.length) {
      downloadCsv(
        `ASC-LUUY-${stamp}.csv`,
        ['STT', 'Loại', 'Module', 'Vấn đề / Màn hình', 'Chi tiết', 'Hướng xử lý'],
  ...
  return (
    <main className="app-shell">
      ...
      <section className="kind-tabs" role="tablist" aria-label="Loại dữ liệu">
        {KIND_TABS.map((tab) => {
          const Icon = tab.icon;
          return (
            <button
      ...
      <footer className="app-footer">
        ...
        <span>
          {configRecords.length} config · {noteRecords.length} lưu ý · tự kiểm tra mỗi {POLL_INTERVAL_MS / 1000}s · v1.2.0
        </span>
      </footer>
