...
export default function App() {
  ...
  const exportCsv = () => {
    const files: string[] = [];
    if (kindView !== 'note' && filteredConfig.length) files.push(`CONFIG (${filteredConfig.length} dòng)`);
    if (kindView !== 'config' && filteredNotes.length) files.push(`Lưu ý (${filteredNotes.length} dòng)`);
