...
type GoogleResponse = {
  ...
  table?: {
    ...
    rows?: GoogleRow[];
    /** Số dòng GViz đã coi là tiêu đề và tách khỏi `rows` */
...
/**
 * Dựng lại danh sách dòng đúng như trên Sheet (phần tử 0 = dòng 1).
 *
 * Dù đã yêu cầu `headers=0`, GViz vẫn có thể tự tách một số dòng đầu ra làm tiêu đề
 * và không đưa vào `rows` — khi đó dữ liệu bị lệch lên trên và dòng đầu tiên bị mất.
 * `parsedNumHeaders` cho biết chính xác số dòng đã bị tách, nhờ đó app bù lại đúng
 * bằng ấy dòng trước khi cắt theo `firstDataRow`, không phụ thuộc vào phỏng đoán.
 */
function extractSheetRows(response: GoogleResponse, size: number) {
  const table = response.table;
  const parsedHeaders = table?.parsedNumHeaders ?? 0;
  const rows: string[][] = [];

  if (parsedHeaders > 0) {
    // Dòng tiêu đề đầu tiên vẫn còn nguyên nội dung trong nhãn cột.
    const labels = (table?.cols || []).slice(0, size).map((col) => String(col?.label ?? '').trim());
    while (labels.length < size) labels.push('');
    rows.push(labels);
    // Các dòng tiêu đề còn lại không lấy lại được, chỉ cần giữ đúng vị trí.
    for (let i = 1; i < parsedHeaders; i += 1) rows.push(new Array<string>(size).fill(''));
  }

  for (const row of table?.rows || []) rows.push(rowToValues(row, size));
  return rows;
}
...
/** Tải một sheet dữ liệu, chỉ lấy đúng các cột cần rồi cắt bỏ phần tiêu đề. */
export async function loadSheet(sheet: SheetDefinition, timeoutMs = 20000): Promise<SheetPayload> {
  // Không dùng `offset` của GViz: phần tiêu đề được cắt ở client sau khi đã bù
  // đúng số dòng GViz tự tách ra, nên không bao giờ lệch dòng.
  const query = `select ${sheet.columns.join(',')}`;
...
export async function loadLookup(timeoutMs = 20000): Promise<LookupPayload> {
  ...
  const rows = extractSheetRows(response, LOOKUP_SHEET.columns.length).slice(
    Math.max(0, LOOKUP_SHEET.firstDataRow - 1),
  );

  const phanHe = cleanLookupColumn(rows.map((row) => row[0]));
  const module = cleanLookupColumn(rows.map((row) => row[1]));
