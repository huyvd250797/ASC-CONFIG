...
/** Kiểm tra token có phải là dãy con theo thứ tự của chuỗi hay không (vd: "cfgmail" ~ "config mail"). */
function isSubsequence(token: string, haystack: string) {
  let index = 0;
  for (const ch of haystack) {
    if (ch === token[index]) index += 1;
...
function scoreToken(token: string, normalized: string) {
  ...
  if (tolerance > 0) {
    for (const word of normalized.split(' ')) {
      ...
      // Gõ thiếu/thừa vài ký tự ở phần đầu của từ dài.
  ...
  if (token.length >= 4 && isSubsequence(token, normalized)) return 34;
